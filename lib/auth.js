...
// User-friendly message for network/Supabase unreachable errors
function normalizeAuthError(error) {
  const msg = error?.message || String(error)
  if (/failed to fetch|network|ERR_NAME_NOT_RESOLVED|Load failed/i.test(msg)) {
    return 'Cannot reach Supabase. Check your internet connection and that your Supabase project is active (unpaused) at supabase.com/dashboard.'
  }
  return msg
}
  ...
  try {
    ...
    if (error) {
      ...
      return {
        ...
        needsEmailConfirmation: false,
        error: { ...error, message: normalizeAuthError(error) },
      }
    }
    ...
    return { user: null, error: { message: normalizeAuthError(error) } }
  }
}
...
export async function signIn(email, password) {
  try {
    ...
    if (error) {
      ...
      return { user: null, error: { ...error, message: normalizeAuthError(error) } }
    }
